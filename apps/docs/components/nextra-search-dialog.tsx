...
type Props = {
	placeholder?: string;
	pages?: PageItem[];
	/** Controlled open state (use with onOpenChange and hideDefaultTrigger for custom trigger) */
	open?: boolean;
	/** Called when dialog open state changes */
	onOpenChange?: (open: boolean) => void;
	/** When true, do not render the default search trigger button */
	hideDefaultTrigger?: boolean;
};
type OnSelect = (url: string) => void;

export function NextraSearchDialog({
	placeholder = "Search...",
	pages = [],
	open: controlledOpen,
	onOpenChange,
		...
		<>
			...
			>
				<div className="border-4 rounded-2xl bg-background">
					<SearchInput placeholder={placeholder} query={query} onQueryChange={setQuery} />
...
function SearchWelcomePages({ pages = [], onSelect }: { pages: PageItem[]; onSelect?: OnSelect }) {
	if (pages.length === 0) {
		return null;
	}
...
function SearchError({ message }: { message: string }) {
	return <div className="py-6 px-4 text-center text-sm text-red-500">{message}</div>;
}
...
function highlightQuery(text: string, query: string) {
	...
	return parts.map((part, index) =>
		regex.test(part) ? (
			<mark key={index} className="bg-primary text-primary-foreground rounded-[3px] font-medium">
				{part}
			</mark>
		) : (
			part
