/**
 * @see https://github.com/shuding/nextra/blob/main/packages/nextra-theme-docs/src/components/navbar/index.client.tsx
 */
...
const NavbarMenu: FC<{
	...
	return (
		<Menu>
			...
			>
				{Object.entries((menu.items as Record<string, { title: string; href?: string }>) || {}).map(
					([key, item]) => (
						<_MenuItem
							key={key}
							as={Anchor}
...
export const ClientNavbar: FC<{
	...
	return (
		<>
			...
			{themeConfig.search && <div className="x:max-md:hidden">{themeConfig.search}</div>}

			{children}

			<Button
				aria-label="Menu"
				className={({ active }) => cn("nextra-hamburger x:md:hidden", active && "x:bg-gray-400/20")}
