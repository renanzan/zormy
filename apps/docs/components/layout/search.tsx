/**
 * @see https://github.com/shuding/nextra/blob/main/packages/nextra/src/client/components/search.tsx#L244
 *
 * @remarks
 * Este componente é uma cópia do componente `Search` do Nextra, mas com a adição de um prop `lang` para filtrar os resultados pelo idioma.
 */
...
import type { PagefindSearchOptions } from "nextra";
...
export const Search: FC<SearchProps> = ({
	...
	useEffect(() => {
		const handleSearch = async (value: string) => {
			...
			const response = await window.pagefind!.debouncedSearch<PagefindResult>(value, searchOptions);
			if (!response) return;
			...
			setResults(
				data
					.filter((newData) => newData.url.startsWith(`/${lang}/`))
					.map((newData) => ({
						...newData,
		...
		<Combobox onChange={handleSelect}>
			...
			>
				{error ? (
					...
					results.map((searchResult) => <Result key={searchResult.url} data={searchResult} />)
				) : (
	...
	return (
		<>
			...
			{data.sub_results.map((subResult) => (
				...
				>
					<div className="x:text-base x:font-semibold x:leading-5">{subResult.title}</div>
					<div
						className={cn(
